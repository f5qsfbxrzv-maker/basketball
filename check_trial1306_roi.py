"""
TRIAL 1306 EDGE OPTIMIZATION
Compare baseline model performance with cleaned odds data
"""
...
# Trial 1306 Features (22 features with multicollinearity)
FEATURES_1306 = [
...
print("="*80)
print("🏁 TRIAL 1306 BASELINE PERFORMANCE")
print("="*80)
...
print(f"\n📚 Training: {len(train_df):,} games")
print(f"🔮 Testing:  {len(test_df):,} games")

# Train Trial 1306 model
print(f"\n⚙️  Training Trial 1306 model...")
...
model = xgb.XGBClassifier(**PARAMS_1306)
...
print(f"✓ Generated predictions for {len(test_clean):,} games")
...
print(f"✓ Merged {len(test_clean):,} games with odds")
...
print("\n" + "="*80)
print("🔍 TRIAL 1306: EDGE THRESHOLD OPTIMIZATION")
print("="*80)
...
# Display results
print("\n" + "="*80)
print("🦁 FAVORITES: TOP 10 THRESHOLDS")
print("="*80)
...
print("\n" + "="*80)
print("🐶 UNDERDOGS: TOP 10 THRESHOLDS")
print("="*80)
print("MIN EDGE     | BETS     | TOTAL UNITS     | ROI        | UNITS/BET")
print("-"*80)
...
print("\n" + "="*80)
print("🎯 TRIAL 1306 OPTIMAL THRESHOLDS")
print("="*80)
print(f"Favorites: {best_fav['edge']*100:.1f}% edge → {best_fav['units']:+.2f} units on {int(best_fav['bets'])} bets ({best_fav['roi']:+.2f}% ROI)")
print(f"Underdogs: {best_dog['edge']*100:.1f}% edge → {best_dog['units']:+.2f} units on {int(best_dog['bets'])} bets ({best_dog['roi']:+.2f}% ROI)")
print(f"COMBINED:  {best_fav['units']+best_dog['units']:+.2f} units on {int(best_fav['bets']+best_dog['bets'])} bets")
print("="*80)
