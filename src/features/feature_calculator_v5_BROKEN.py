...
class FeatureCalculatorV5:
    ...
    def calculate_game_features(
        ...
        # 1. TRADITIONAL FEATURES (Keep for baseline)
        features['injury_impact_diff'] = home_injury - away_injury
        features['injury_impact_abs'] = abs(home_injury) + abs(away_injury)
        
        # 2. INJURY SHOCK (New News vs EWMA Baseline)
        # Get EWMA injury impact for both teams
        # Convert datetime to string format for SQL query
        ...
        # Shock = Today's injury - Rolling average (positive = worse than usual)
        features['injury_shock_home'] = home_injury - home_ewma_inj
        features['injury_shock_away'] = away_injury - away_ewma_inj
        features['injury_shock_diff'] = features['injury_shock_home'] - features['injury_shock_away']
        
        # 3. STAR POWER BINARY FLAGS ("Loud" Signal for Trees)
        # Threshold: PIE >= 0.15 implies ~3.0+ impact, top 3 rotation player
        # Conservative: Use 4.0 threshold (elite starters only)
        STAR_THRESHOLD = 4.0
        features['home_star_missing'] = 1 if home_injury >= STAR_THRESHOLD else 0
        ...
        # ====================================================================
        # SEASON CONTEXT FEATURES (Required by model)
        # ====================================================================
        # Extract season info from game_date and season string
        # Season format: "2024-25" (starts in October 2024, ends in June 2025)
        try:
            season_year = int(season.split('-')[0])  # e.g., 2024 from "2024-25"
            features['season_year'] = season_year
            features['season_year_normalized'] = (season_year - 2020) / 10.0  # Normalize around recent years
            
            # Calculate games_into_season and season_progress
            # NBA season starts ~Oct 15, playoffs end ~June 15 (roughly 240 days)
            season_start = datetime(season_year, 10, 15)
            season_end = datetime(season_year + 1, 6, 15)
            total_season_days = (season_end - season_start).days
            days_into_season = max(0, (current_date - season_start).days)
            
            # Season progress: 0.0 (opening day) → 1.0 (finals)
            features['season_progress'] = min(1.0, max(0.0, days_into_season / total_season_days))
            
            # Games into season: rough estimate based on ~82 games over 180 days (Oct 15 - Apr 15)
            # After Apr 15, it's playoffs
            features['games_into_season'] = min(82, int(days_into_season * 82 / 180))
            
            # Is season opener? (first week of season)
            features['is_season_opener'] = 1 if days_into_season <= 7 else 0
            
            # Season month (1-12)
            features['season_month'] = current_date.month
            
            # Endgame phase: Last 2 weeks of regular season (crucial for playoff positioning)
            # Regular season ends mid-April (~180 days into season)
            features['endgame_phase'] = 1 if 166 <= days_into_season <= 180 else 0
            
        except Exception as e:
            # Fallback to reasonable defaults if date parsing fails
            self.logger.warning(f"Could not calculate season features: {e}")
            features['season_year'] = 2024
            features['season_year_normalized'] = 0.4
            features['season_progress'] = 0.5
            features['games_into_season'] = 41
            features['is_season_opener'] = 0
            features['season_month'] = 12
            features['endgame_phase'] = 0
        
        # SHAP-based feature pruning: DISABLED to preserve all model features
    ...
    def _calculate_injury_impact(self, home_team: str, away_team: str) -> Dict:
        ...
        try:
            ...
            return {
                'home_injury_impact': home_loss * 100.0,
                'away_injury_impact': away_loss * 100.0
            }
        except Exception as e:
