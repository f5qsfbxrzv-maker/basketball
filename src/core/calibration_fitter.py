...
class CalibrationFitter:
    ...
    **CRITICAL**: All betting probabilities MUST pass through `apply()` before Kelly sizing.
    ...
    Examples:
        >>> fitter = CalibrationFitter(db_path='nba_betting_data.db', min_samples=250)
    ...
    """
    def __init__(self, db_path: str = 'nba_betting_data.db', min_samples: int = MIN_CALIBRATION_SAMPLES):
        """
        Initialize calibration fitter with database connection and minimum sample threshold.
        
        Args:
            db_path (str): Path to SQLite database. Default: 'nba_betting_data.db'
